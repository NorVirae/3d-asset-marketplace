...
const FileManager = () => {
        ...
        const buySpace = async (size) => {
                const spaceToBuy =  Number(size) * 1024 * 1024
                try{
                        let url = "http://localhost:8080/file/buy/storage"
                        const result = await axios.post(url, {spaceToBuy, id:user._id})
                        alert(result.data.message)
                        let resData = result.data.data
                        dispatch({type:"LOGGED_IN", payload:{...user, storage:resData.storage, totalAccumulatedStorage:resData.totalAccumulatedStorage} })
                        console.log(result)
                }catch(err){
                        console.log(err)
        ...
        <div className="container">
                ...
                <main className="file-main"> 
                        <div className="file-left">
                                ...
                                <div className="remaining">Remaining: {user?convertToMB(user.totalAccumulatedStorage):"--"}MB </div>
                                ...
                                <button onClick={e=>buySpace(50)} className="buy-space-btn">
                                       Buy Space!
                                </button>
