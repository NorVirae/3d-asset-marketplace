...
const NavBar = ({search, style, shortSearch, loggedIn}) => {
    ...
    return (
        <nav style={{...style, background: `${yScroll>50?"#232230": "transparent"}`, padding: `${yScroll > 50? "1rem": "2rem"}`, width: `${setShowRegModal.isSidebarOpen? "82vw": "100%"}`, right: `${setShowRegModal.isSidebarOpen? "-19%": "100%"}`, position: `${yScroll > 400? "fixed": "sticky"}`}} className="landing__navbar">
            <div className="landing__sidebar-toggle" style={{display: "flex", justifyContent: "center", alignItems: "center", gap: "2rem"}}>
                ...
                {search && <SearchBar style={{backgroundColor: "red"}} short={shortSearch}/>}
            </div>
            
            <Link className="landing__navbar-brand" to = "/">
                {/* <LoootyLogo color={"white"} style={{marginLeft: shortSearch? "-90%": "200%"}}/> */}
                <LoootyLogoSplit color={"white"} style={{marginLeft: shortSearch? "-90%": "200%"}}/>
            </Link>
