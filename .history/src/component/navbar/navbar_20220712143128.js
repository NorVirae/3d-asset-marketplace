...
const NavBar = ({search, style, shortSearch, loggedIn}) => {
    ...
    return (
        <nav style={{...style, background: `${yScroll>50?"#232230": "transparent"}`, width: `${setShowRegModal.isSidebarOpen? "82vw": "100%"}`, right: `${setShowRegModal.isSidebarOpen? "-19%": "0%"}`, position: `${yScroll > 400? "fixed": "sticky"}`}} className="landing__navbar">
            <div className="landing__sidebar-toggle" style={{display: "flex", justifyContent: "center", alignItems: "center", gap: "2rem"}}>
                ...
                {search && <div></div><NavSearchBar short={shortSearch}/>}
            </div>
            
            <Link className="landing__navbar-brand" to = "/">
                {/* <LoootyLogo color={"white"} style={{marginLeft: shortSearch? "-90%": "200%"}}/> */}
                <LoootyLogo color={"white"} style={{marginLeft: shortSearch? "-80%": "191%"}}/>
            </Link>
