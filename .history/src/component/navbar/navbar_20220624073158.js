    ...
    return (
        <nav style={{...style, background: `${yScroll>50?"#232230": "transparent"}`, padding: `${yScroll > 50? "1rem": "2rem"}`, width: `${yScroll > 400? "82vw": "100%"}`, right: `${yScroll > 400? "-19%": "100%"}`, position: `${yScroll > 400? "fixed": "sticky"}`}} className="landing__navbar">
            <div className="landing__sidebar-toggle" style={{display: "flex", justifyContent: "center", alignItems: "center", gap: "2rem"}}>
                {!showRegModal.isSidebarOpen?<GoThreeBars className="landing__sidebar-toggler" onClick={() => showRegModal.setIsSidebarOpen(old => ({...old, isSidebarOpen:true}))} style={{color: "white", fontSize: "2rem"}}/>:<FaTimes className="landing__sidebar-toggler" onClick={() => showRegModal.setIsSidebarOpen(old => ({...old, isSidebarOpen:false}))} style={{color: "white", fontSize: "2rem"}} />}
